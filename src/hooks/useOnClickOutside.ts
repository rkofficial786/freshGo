...
export default function useOnClickOutside(ref, triggerElement, handler) {
  useEffect(() => {
    // Define the listener function to be called on click/touch events
    const listener = (event) => {
      // If the click/touch event originated inside the ref element or the trigger element, do nothing
      if (
        (!ref.current || ref.current.contains(event.target)) ||
        triggerElement.current.contains(event.target)
      ) {
        return;
      }
  ...
  }, [ref, triggerElement, handler]); // Only run this effect when the ref, trigger element, or handler function changes
}
